...
export function renderDiagnostics() {
  ...
  function paintLog(lines) {
    if (!lines.length) {
      if (logEl.dataset.content !== '') {
        logEl.dataset.content = ''
    ...
    // وقتی محتوا عوض نشده، DOM را دست نمی‌زنیم — نه reflow نه بازنویسی.
    if (logEl.dataset.content !== html) {
      logEl.dataset.content = html
  ...
  root.querySelector('#run').addEventListener('click', async () => {
    try {
      ...
      overallDot.style.background = CHECK_COLOR.FAIL
      overallText.textContent = String(e)
    }
  })
  ...
  root.querySelector('#copy').addEventListener('click', async () => {
    try {
      ...
      ta.select()
      // navigator.clipboard در وب‌ویو Tauri دردسترس است؛ fallback لِگسی
      // execCommand فقط برای محیط‌های محدود محفوظ است.
      try {
  ...
  // renderTab() در main.js این تابع پاک‌سازی را پیش از حذف ویو صدا می‌زند.
