...
// شمارندهٔ ترتیب جابه‌جایی تب — اگر کاربر سریع چند تب را بزند، فقط آخرین
// جابه‌جایی اعمال می‌شود و انیمیشن‌های کهنهٔ خروج وسطِ کار اعمال نمی‌شوند.
let tabSeq = 0
function renderTab() {
  ...
  // پاک‌سازی پیوست‌به‌نما (unsubscribe از onChange + تایمرها) پیش از حذف
  // DOM. قبلاً `listeners.clear()` این‌جا داشت — تبری که لیسنرهای ثبت‌شدهٔ
  // بیرون از ویو را هم نابود می‌کرد؛ حالا هر ویو پاک‌سازی خودش را دارد.
  const apply = () => {
  ...
  // خروج نرم (fade) پیش از جابه‌جایی؛ ورودِ slide نرم بعداً با CSS می‌آید.
  if (old && typeof old.animate === 'function' && !reduced) {
...
async function boot() {
  ...
  // هر شکست، همان دستور را fail می‌کند نه هر دو؛ پوسته هم ساده دربسته نمی‌میرد.
  const [profile, snapshot] = await Promise.all([
    invoke('get_profile').catch((e) => {
      console.error('get_profile failed', e)
      return null
    }),
    invoke('get_snapshot').catch((e) => {
      console.error('get_snapshot failed', e)
      return null
    }),
  ])
  if (!profile) {
    host.innerHTML = '<p class="view__lead" style="color:var(--aether-error)">' + t('Could not load your settings. Please restart Aether.') + '</p>'
    return
  }
  app.profile = profile
  app.snapshot = snapshot ?? app.snapshot
