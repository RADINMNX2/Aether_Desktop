...
mod tests {
    ...
    #[test]
    fn hostile_network_leads_with_the_hardened_pass() {
        // ریشهٔ باگ کانکت نشدن روی شبکهٔ فیلترشده: پاس اولِ ساده نباید
        // پنجرهٔ مهلت را بسوزاند. در حالت hostile هر دو پلن (دستی و خودکار)
        // باید با پاس سخت‌شده شروع شوند.
        let manual = ConnectionProfile { protocol: Protocol::Masque, ..Default::default() };
        let mplan = build_plan(&manual, true);
        assert_eq!(mplan.len(), 2);
        assert_eq!(mplan[0].profile.noize, Noize::Firewall, "manual: hardened pass must lead on a filtered network");
        assert_eq!(mplan[0].profile.fragment, true);

        let smart = ConnectionProfile::default();
        let splan = build_plan(&smart, true);
        assert_eq!(splan[0].profile.noize, Noize::Firewall, "auto: hardened pass must lead on a filtered network");
    }

    #[test]
    fn hostile_network_never_emits_smart_protocol() {
        let smart = ConnectionProfile::default();
        for c in build_plan(&smart, true) {
            assert_ne!(c.profile.protocol, Protocol::Smart);
        }
    }

    #[test]
    fn ipv6_only_reorders_the_ladder_for_wireguard() {
        let p = ConnectionProfile { ip_version: IpVersion::V6, ..Default::default() };
        let plan = build_plan(&p, false);
        // ترتیب: WireGuard (اول با دو پاس) → MASQUE سخت‌شده → GOOL سخت‌شده.
        assert_eq!(plan[0].profile.protocol, Protocol::Wireguard);
        assert_eq!(plan[1].profile.protocol, Protocol::Wireguard);
        assert_eq!(plan[2].profile.protocol, Protocol::Masque);
        assert_eq!(plan[3].profile.protocol, Protocol::Gool);
        for c in &plan {
            assert_ne!(c.profile.protocol, Protocol::Smart);
        }
    }
    ...
    #[test]
    fn masque_hardening_keeps_protocol() {
        // harden فقط نویز/فلاگ را عوض می‌کند؛ پروتکل هرگز تغییر نمی‌کند.
        let p = ConnectionProfile { protocol: Protocol::Masque, ..Default::default() };
        for c in build_plan(&p, true) {
            assert_eq!(c.profile.protocol, Protocol::Masque);
        }
    }
}
