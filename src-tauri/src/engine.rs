...
impl AetherProcess {
    ...
    /// ویندوز سیگنال ترک‌ناپذیر برای فرآیند فرزند ندارد؛ poll سبک با
    /// `try_wait` (هر ۲۰ms) بدون اثر بر مسیر داده فقط همین کار را می‌کند.
    ...
    /// معادل `stop()`: در ویندوز SIGTERM وجود ندارد، پس TerminateProcess
    /// بلافاصله صدا زده می‌شود. پنجرهٔ 250ms فقط مهلت reap فرآیند است، نه
    /// مهلت خاتمهٔ مؤدبانه. برگشت از این تابع یعنی فرآیند واقعاً reap شده است.
    pub fn stop(&mut self) {
        let Some(mut child) = self.child.take() else { return };
        let _ = child.kill(); // در ویندوز TerminateProcess فوری است
        let deadline = Instant::now() + Duration::from_millis(REAP_WAIT_MS);
        while Instant::now() < deadline {
